...
function checkCommands() {
    ...
    for (const file of listed) {
        if (!commands.includes(file)) {
            fail(`README lists /${file.replace(/\.md$/, '')} but commands/${file} is missing`)
        }
    }

    for (const file of commands) {
        if (!listed.includes(file)) {
            fail(`commands/${file} exists but README does not list /${file.replace(/\.md$/, '')}`)
        }
    }
}
...
function checkRouterTargets() {
    const expected = [
        ...
        ['commands/tdd.md', ['agents/tdd-guide.md', 'skills/test-driven-development/SKILL.md']],
        ['commands/e2e.md', ['agents/e2e-runner.md', 'skills/e2e-testing/SKILL.md']],
...
function checkObserveV2() {
    ...
    if (!fs.existsSync(config)) return fail('continuous-learning-v2 config is missing')

    const tempHome = rel('.tmp-verify-harness')
    const tempConfigDir = path.join(tempHome, '.claude', 'skills', 'continuous-learning-v2')
    const observationsPath = path.join(tempHome, '.claude', 'homunculus', 'observations.jsonl')
    ...
    fs.writeFileSync(path.join(tempConfigDir, 'config.json'), JSON.stringify(cfg, null, 2))
    ...
    if (result.error) fail(`observe-v2 smoke test failed: ${result.error.message}`)
    if (result.status !== 0) fail(`observe-v2 exited ${result.status}: ${result.stderr}`)
    if (!fs.existsSync(observationsPath)) {
        fail('observe-v2 did not write observations.jsonl')
    } else {
        const firstLine = fs.readFileSync(observationsPath, 'utf8').split(/\r?\n/)[0]
        try {
