...
function checkCapabilityEntries(manifest, manifestPath) {
  ...
  for (const entry of capabilities) {
    const excluded = ["wayfinder", "setup-matt-pocock-skills"].includes(entry.upstream) && entry.status === "excluded" &&
      typeof entry.reason === "string" && entry.reason.trim().length > 0;
    if (!excluded && !["covered", "adapted"].includes(entry.status)) {
...
function checkCapabilityRouting() {
  requireTokens("skills/using-superpowers/SKILL.md", [
    ...
    "external engineering fact or documentation question?",
...
function checkDeliveryCapabilityContracts() {
  if (exists("skills/research/SKILL.md")) fail("standalone research Skill must be retired");
  const retired = JSON.parse(read("scripts/retired-skill-files.json"));
  if (!retired.retired_skills?.research?.includes("SKILL.md")) {
    fail("research retirement must clean up the installed entrypoint");
  }
...
function checkRemovedSkillReferences() {
  ...
  requireTokens("rules/common/skills-learning.md", [
    "Skill 创建和更新遵循本规则",
    "开放生态中的 skill 查找和安装按用户明确请求使用宿主已有工具",
  ]);
}
