...
本文件只保留专项规则的触发矩阵；命中场景时再读取对应规则。

## 加载层级

1. 入口层：`AGENTS.md` 或 `CLAUDE.md` 的硬规则和规则索引。
2. 基础层：实现、审查或验证时读取 `01-base.md` 与少量相关基础规则。
3. 专项层：任务明确触发时读取触发矩阵中的对应规则文件。

不得因为“可能有用”而一次性读取完整 `rules/` 或 `rules/common/`。

## 默认原则

简单任务由当前 agent 直接完成，不加载完整专项规则集；只有复杂、跨领域或需要独立视角时才加载对应专项规则并考虑委派。长任务维护干净工作集，subagent 只回传结论和证据；Agent 不稳定时先检查 Harness 六层：主循环、工具、上下文、状态、权限、验证。
